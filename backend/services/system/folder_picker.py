...
POWERSHELL_MODERN_SCRIPT = """
$code = @'
using System;
using System.Runtime.InteropServices;

namespace FastPicker {
...
[FastPicker.FolderPicker]::Show()
"""

_encoded_ps = base64.b64encode(POWERSHELL_MODERN_SCRIPT.encode('utf-16le')).decode('ascii')

def open_modern_folder_picker() -> str | None:
    """
    Opens modern Windows File Explorer folder picker dialog (IFileOpenDialog with FOS_PICKFOLDERS).
    Thread-safe non-blocking lock guarantees ONLY ONE SINGLE DIALOG opens at a time (NO double-opening).
    """
    if not _picker_lock.acquire(blocking=False):
        logger.warning("Folder picker is already active, ignoring duplicate request.")
        return None
    ...
    try:
        ...
        logger.error(f"Modern folder picker error: {e}")
    finally:
