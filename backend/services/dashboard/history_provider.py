...
class HistoryProvider:
    ...
    def get_history(db: Session, limit: int = 5) -> dict:
        completed = db.query(
            UploadTask.id, UploadTask.title, UploadTask.channel_id, UploadTask.video_id, UploadTask.status, UploadTask.completed_at
        ).filter(
            UploadTask.status == QueueStatusEnum.completed
        ).order_by(desc(UploadTask.completed_at)).limit(limit).all()
        
        failed = db.query(
            UploadTask.id, UploadTask.title, UploadTask.channel_id, UploadTask.video_id, UploadTask.status, UploadTask.completed_at, UploadTask.created_at
        ).filter(
        ...
        return {
            "latest_completed": [format_task(t, channels.get(t.channel_id, "YouTube")) for t in completed],
            "latest_failed": [format_task(t, channels.get(t.channel_id, "YouTube")) for t in failed],
