...
Architecture (Stage 3.5.2-R1 locked):
  ...
  - Per-account scan loop (queries DB for enabled accounts each cycle)
...
class WatchFolderEngine(EngineBase):
    ...
    def scan_now(self, account_id: str = None, pipeline_type: str = None) -> ScanSummary:
        logger.info(f"[ENGINE] scan_now() triggered manually | account_id={account_id} | pipeline_type={pipeline_type}")
        summary = ScanSummary()
        db = SessionLocal()
        try:
        ...
        for account in enabled_accounts:
            acc_summary = self._scan_account(account, db, target_pipeline=pipeline_type)
    ...
    def _run_cycle(self):
        ...
        for account in enabled_accounts:
            ...
            db = SessionLocal()
            try:
                self._scan_account(account, db)
            finally:
                db.close()

    def _scan_account(self, account: Account, db: Session, target_pipeline: str = None) -> ScanSummary:
        summary = ScanSummary()
        ...
        if not pipelines and account.watch_folder_enabled and account.watch_folder:
            ...
            account.pipelines = json.dumps(pipelines)
            db.commit()
        ...
        for p_key, p_config in pipelines.items():
            ...
            health_service.update_status(account_id, p_key, "SCANNING")
            health_service.set_watch_folder_path(account_id, p_key, watch_path)
            health_service.record_log(account_id, p_key, "PASS", "Pipeline Configured")
            health_service.record_scan(account_id, p_key)
            ...
            if not path_ok:
                health_service.record_path_unavailable(account_id, p_key)
                health_service.record_scan_result(account_id, p_key, "PATH_UNAVAILABLE", 0)
                health_service.record_log(account_id, p_key, "FAIL", "Folder Not Found")
                continue

            health_service.record_log(account_id, p_key, "PASS", "Folder Exists")

            if not candidates:
                health_service.record_scan_result(account_id, p_key, "NO_PACKAGES", 0)
                health_service.update_status(account_id, p_key, "IDLE")
                continue
                
            summary.packages_found += len(candidates)
            health_service.record_log(account_id, p_key, "PASS", f"Packages Found ({len(candidates)})")

            # Known tasks
            from schemas import QueueStatusEnum
            known_tasks = db.query(UploadTask).filter(UploadTask.account_id == account_id).all()
            locked_paths = set()
            ...
            for folder_entry in valid_candidates:
                ...
                if not result.success:
                    health_service.record_error(account_id, p_key, result.error_code)
                    health_service.record_log(account_id, p_key, "FAIL", f"Validation Failed: {result.error_code}")
                    p_errors += 1
                    locked_paths.add(folder_path)
                    summary.validation_errors += 1
                    continue

                health_service.record_log(account_id, p_key, "PASS", "Package Validated")
                ...
                if dup_result.is_duplicate:
                    health_service.record_error(account_id, p_key, "DUPLICATE_SKIPPED")
                    health_service.record_log(account_id, p_key, "FAIL", "Duplicate Package")
                    p_dupes += 1
                    locked_paths.add(folder_path)
                    summary.duplicates_skipped += 1
                    continue

                health_service.record_log(account_id, p_key, "PASS", "Duplicate Check")

                try:
                    task = importer.create_task(result, account, db, p_key, p_config, today_intake=state.get("today_intake", 0))
                    p_imported += 1
                    summary.tasks_created += 1
                    state["today_intake"] = state.get("today_intake", 0) + 1
                    state_modified = True
                    health_service.record_log(account_id, p_key, "PASS", "UploadTask Created")
                    health_service.record_log(account_id, p_key, "PASS", "Database Commit")
                    
                    pipeline_states[p_key] = state
                    account.pipeline_states = json.dumps(pipeline_states)
                    db.commit()
                except Exception as e:
                    logger.error(f"[ENGINE] Import failed | folder={folder_path!r} | error={e}")
                    health_service.record_error(account_id, p_key, "IMPORT_FAILED")
                    health_service.record_log(account_id, p_key, "FAIL", "Import Failed")
                    p_errors += 1
                    summary.validation_errors += 1
            ...
            if p_imported > 0:
                health_service.record_scan_result(account_id, p_key, "IMPORTED", p_imported)
            elif p_dupes > 0:
                health_service.record_scan_result(account_id, p_key, "DUPLICATE", p_dupes)
            elif p_errors > 0:
                health_service.record_scan_result(account_id, p_key, "VALIDATION_FAILED", p_errors)
            else:
                health_service.record_scan_result(account_id, p_key, "NO_PACKAGES", 0)
